function varargout = calc_CVLOO(core,dcore)
%% CALC_CVLOO Calculates the cross-validation likelihood via leave-one-out
arguments (Input)
    core struct;
    dcore struct = struct();
end
    ...
    for idx = 1:numel(Y)
        %Used parameters
        Q_p = Q(idx,idx); % Kinv_ii
        alpha_p = alpha(idx);

        %Cholesky decomposition of submatrix
        P_p = sqrt(Q_p+1e-9.*norm(Q_p));
        %P_p = chol(Q_p,'lower');
        
        %Predictive likelihood
        CV_nlpl_p = 0.5*(alpha_p'*(P_p'\(P_p\alpha_p)) - 2*sum(log(diag(P_p))) + log(2*pi));
        ...
        if nargout >1
            ...
            for jdx = 1:numel(hyp_names)
                %Used parameters
                ZQ_p = ZQ.(hyp_names{jdx})(idx,idx);
                Zalpha_p = Zalpha.(hyp_names{jdx})(idx);

                %Derivative of predictive likelihood
                S = P_p'\(P_p\(ZQ_p*(P_p'\(P_p\1))));
